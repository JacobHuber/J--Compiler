...
void Parser::globaldeclaration() {
	...
	switch (lookAhead.getType()) {
		...
		case IDENTIFIER:
			...
			break;
		default:
			std::cerr << "ERROR: Expected token of type 'BOOLEAN, INT, VOID, or IDENTIFER on line " << lookAhead.getLineNumber() << "\n";
			exit(-1);
			break;
	}
}
...
Node Parser::conditionalorexpression() {
	...
	Node op = conditionalorexpression2();
...
Node Parser::conditionalorexpression2() {
	Node op;
	fetch();
	...
	if (op.getChildren().size() > 0) {
		Node op2 = conditionalorexpression2();
...
Node Parser::conditionalandexpression() {
	...
	Node op = conditionalandexpression2();
	if (op.getChildren().size() > 0) {
		op.addChildFront(eq);
		return op;
	}
...
Node Parser::conditionalandexpression2() {
	Node op;
	...
	if (op.getChildren().size() > 0) {
		Node op2 = conditionalandexpression2();
...
Node Parser::equalityexpression2() {
	Node op;
...
Node Parser::relationalexpression2() {
	Node op;
	...
	if (op.getChildren().size() > 0) {
		Node op2 = relationalexpression2();
...
Node Parser::additiveexpression2() {
	Node op;
...
Node Parser::multiplicativeexpression() {
	...
	Node op = multiplicativeexpression2();
	if (op.getChildren().size() > 0) {
...
Node Parser::multiplicativeexpression2() {
	Node op;
	...
	if (op.getChildren().size() > 0) {
		Node op2 = multiplicativeexpression2();
